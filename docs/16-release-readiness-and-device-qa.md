...
本文档用于冻结发布前检查。当前结论：上海 2015-2025 公开支持范围已可进入测算，正式发布仍被发布资产和真机链路阻塞。
...
状态快照还会展示 `Release readiness next actions`，按 `git`、`data`、`qa` 拆分 P0 行动；当前 P0 主要对应发布资产与发布支持文件跟踪、微信开发者工具预览和真机证据补齐。机器可读 JSON 同步包含 `nextActions` 字段。
...
- 发布资产已纳入 git，且没有本地未提交改动。
- 至少一个城市对普通用户开放测算。
- 微信开发者工具预览和真机 QA 证据通过。

总闸门失败时会聚合关键下一步，包括发布资产和发布支持文件 `git add` 命令、微信开发者工具预览命令和真机 QA 证据文件路径。正式发版前必须让该命令通过。
...
当资产存在但未进入版本控制，或已跟踪资产仍有本地未提交改动时，该命令会输出需要执行的 `git add` 命令。当前沙箱环境可能无法写入 `.git/index.lock`，需要在可写 git 环境中跟踪并提交这些发布资产后再复查。
...
- 覆盖 iOS 和 Android。
- 确认首页入口、城市闸门、表单校验、账户余额流程、上海账户查询路径、结果解释、数据来源/隐私/免责声明、分享隐私、大字号、键盘安全区等检查项。
...
- 北京、深圳仍保持不可公开测算。
- 发布资产和发布支持文件尚未全部进入版本控制。
- 小程序必须通过微信开发者工具真实编译、预览和真机扫码验收。
- 真机验收后必须留下 `qa/device-qa-evidence.json` 证据文件。
- `miniprogram/assets/release-assets.json` 和清单内发布资产必须进入版本控制，避免线上包缺图。
...
- 如果本次输入触达 2000-2007、2013 等仍在核对年份，会阻止该组测算并提示历史年份仍在核对。
...
这些缺口未补齐前，触达对应历史年份的输入不能对普通用户开放测算。
...
- 上海可进入测算；若已缴年限触达仍在核对的历史年份，普通用户不能提交该组测算。
- 女性退休类型按两列布局展示。
